...
function getProviderConfig(modelId: string): { order: string[]; allow_fallbacks: boolean } {
  ...
  // Default to OpenAI
  return { order: ["openai"], allow_fallbacks: false };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get the pitch deck text
    const pitchDeckText = submission.pitchDeckFile?.extractedText || "";

    if (!pitchDeckText) {
      return NextResponse.json(
        { error: "Pitch deck text not found" },
        { status: 500 }
      );
    }

    // Truncate if too long
    let content = pitchDeckText;
